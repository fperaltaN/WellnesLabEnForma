...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if(this.tagName.toUpperCase() !== 'TABLE' || this.id == null) {
			console.log("Element is not a table or has no id!");
			return;
		}
		if(document.documentMode !== undefined ) { // IE only
			if(document.documentMode <= 5) {
				console.log("Grid can not be used in this ('quirks') mode!");
				return;
			}
		}
		var i =0, lr, lk, dir;
		...
		if( this.p.colNames.length !== this.p.colModel.length ) {
			console.log($.jgrid.getRegional(this,"errors.model"));
			return;
		}
...
$.extend($.jgrid,{
		...
		if(o.useStorage && $.jgrid.isLocalStorage()) {
			try {
				...
				if(e.code === 22) { // chrome is 21
					...
					console.log("Local storage limit is over!");
				}
			}
		}
		return gridstate;
	...
	loadState : function (jqGridId, gridstring, o) {
		...
		if( ret && $.type(ret) === 'object') {
			...
			console.log("can not convert to object");
		}
	},
	...
	jqGridImport : function(jqGridId, o) {
		...
		var xmlConvert = function (xml,o) {
			...
			if($.grid.xmlToJSON ) {
				...
				console.log("xml2json or parse are not present");
			}
		};
		...
		if($.ui) {
			if ($.ui.multiselect && $.ui.multiselect.defaults) {
				if (!$.jgrid._multiselect) {
					...
					console.log("Multiselect plugin loaded after jqGrid. Please load the plugin before the jqGrid!");
					return;
				}
